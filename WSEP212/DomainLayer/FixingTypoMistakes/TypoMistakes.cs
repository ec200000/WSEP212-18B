...
namespace WSEP212.DomainLayer.FixingTypoMistakes
{
    ...
    /// </summary>
    public class TypoMistakes
    {
        private static readonly Lazy<TypoMistakes> lazy
        = new Lazy<TypoMistakes>(() => new TypoMistakes());

        public static TypoMistakes Instance
            => lazy.Value;
            ...
            foreach (var word in wordList)
            {
                ...
                if (_wordRegex.IsMatch(trimmedWord))
                {
                    if (_dictionary.ContainsKey(trimmedWord))
                        ...
                        _dictionary.TryAdd(trimmedWord, 1);
                }
            }
        }
